...
const Checkout = () => {
  ...
  const addDebugLog = (log) => {
    if (debugMode) {
      setDebugLogs((prev) => [...prev, { timestamp: new Date().toISOString(), ...log }].slice(-10));
    }
  };
  ...
  useEffect(() => {
    const loadCartAndUserData = async () => {
      try {
        ...
        addDebugLog({ type: 'cart_load', data: cartItems });
        ...
        const processedCart = await Promise.all(
          cartItems.map(async (item) => {
            ...
            if (filteredImageUrls.length === 0) {
              ...
              console.warn('No valid imageUrls for product:', productData.name || 'Unknown', 'Using placeholder');
            }
            ...
            const categoryFees = feeSnap.exists() && feeSnap.data()[category] || {
              taxRate: 0.075,
              buyerProtectionRate: 0.02,
              handlingRate: 0.05,
            };
            const basePrice = productData.price || 0;
            const totalPrice = basePrice * (1 + categoryFees.taxRate + categoryFees.buyerProtectionRate + categoryFees.handlingRate);
  ...
  const sendOrderConfirmationEmail = async (order) => {
    try {
      ...
      const errorDetails = {
        ...
        response_data: err.response?.data ? JSON.stringify(err.response.data, null, 2) : 'No response data',
        stack: err.stack,
      };
      console.error('Error sending order confirmation email:', errorDetails);
      console.log('Logging to Sentry:', err);
      addDebugLog({ type: 'email_error', data: errorDetails });
      toast.warn(
        'Order placed successfully, but failed to send confirmation email. Please check your email later or contact support@foremade.com.',
        {
          position: 'top-right',
          autoClose: 5000,
        }
      ...
      try {
        ...
        await runTransaction(db, async (transaction) => {
          ...
          productSnaps.forEach((snap, index) => {
            ...
            if (newStock < 0) {
              throw new Error(
                `Insufficient stock for product ${cart[index].productId} (${cart[index].product?.name || 'Unknown'})`
              );
            }
        ...
        toast.success(
          <div>
            ...
            <p>Your order has been placed. Check your email for confirmation from Foremade.</p>
          </div>,
          {
            position: 'top-right',
            autoClose: 5000,
  ...
  const handleImageError = (e, productId) => {
    ...
    if (debugMode) {
      toast.warn(`Image failed to load for product ${productId}`, { position: 'top-right', autoClose: 3000 });
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8 relative">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="flex-1 bg-white p-6 rounded-lg shadow-md" ref={formRef}>
            ...
            <form className="space-y-4">
              ...
              <div>
                <label htmlFor="postalCode" className="block text-sm font-medium text-gray-700 flex items-center gap-1">
                ...
                {formErrors.postalCode && (
                  <p id="postalcode-error" className="text-red-600 text-xs mt-1">
                    {formErrors.postalCode}
                  </p>
                )}
              </div>
              <div>
                <label htmlFor="country" className="block text-sm font-medium text-gray-700 flex items-center gap-1">
                ...
                <div key={item.productId} className="flex items-start gap-4 mb-4 border-b pb-4">
                  <div className="w-24 h-24 relative overflow-hidden rounded-lg image-zoom">
                    ...
                    <img
                      ...
                      loading="lazy"
                      aria-label={`Image for ${item.product?.name || 'Unknown Product'}`}
                    />
                ...
                <div className="mt-6">
                  ...
                  {formData.country === 'United Kingdom' ? (
                    ...
                    <PaystackCheckout
                      ...
                      aria-label="Pay Now with Paystack"
      ...
      {debugMode && (
        <div className="fixed bottom-4 right-4 bg-gray-800 text-white p-4 rounded-lg shadow-lg max-h-96 overflow-y-auto max-w-3xl">
          <h3 className="font-bold mb-2 text-blue-300">Debug Logs</h3>
          <pre className="text-xs">{JSON.stringify({ state: { cart, formData, formErrors, imageLoading, imageErrors }, debugLogs }, null, 2)}</pre>
        </div>
      )}
    </div>
  );
