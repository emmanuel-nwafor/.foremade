...
export default function BabyProducts() {
  ...
  const categoryId = 10; // Assuming a unique categoryId for Baby Products
  const firestoreCategory = 'baby products';
  ...
  useEffect(() => {
    const fetchBabyProducts = async () => {
      try {
        ...
        console.log('All fetched products (Baby Products):', allProducts);
        ...
        const productsData = allProducts
          .map((doc) => {
            ...
            return {
              ...
              price: data.price || 0,
              stock: data.stock || 0,
              category: data.category || 'baby products',
              categoryId: data.category?.trim().toLowerCase() === firestoreCategory.toLowerCase() ? categoryId : 10,
              colors: data.colors || [],
              sizes: data.sizes || [],
          ...
          .filter((product) => {
            if (product.stock < 10) {
              console.warn('Filtered out product with low stock (Baby Products):', {
                id: product.id,
                name: product.name,
                stock: product.stock,
              });
            ...
            if (!isValidImage && product.imageUrl !== '/images/placeholder.jpg') {
              console.warn('Filtered out product with invalid imageUrl (Baby Products):', {
                id: product.id,
                name: product.name,
        ...
        if (productsData.length === 0) {
          console.warn('No products passed the filters (Baby Products). Relaxing stock filter to debug...');
          const relaxedProductsData = allProducts
            ...
            .filter((product) => {
              const isValidImage = product.imageUrl && typeof product.imageUrl === 'string' && product.imageUrl.startsWith('https://');
              if (!isValidImage && product.imageUrl !== '/images/placeholder.jpg') {
                console.warn('Filtered out product with invalid imageUrl (relaxed filter, Baby Products):', {
                  id: product.id,
                  name: product.name,
                  imageUrl: product.imageUrl,
                });
          ...
          console.log('Products with relaxed stock filter (Baby Products):', relaxedProductsData);
