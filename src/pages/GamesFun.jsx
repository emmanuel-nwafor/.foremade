...
export default function GamesFun() {
  ...
  useEffect(() => {
    const fetchGamesFun = async () => {
      try {
        ...
        console.log('All fetched products (Games & Fun):', allProducts);
        ...
        const productsData = allProducts
          .map((doc) => {
            ...
            return {
              ...
              price: data.price || 0,
              stock: data.stock || 0,
              category: data.category || 'games & fun',
              categoryId: data.category?.trim().toLowerCase() === firestoreCategory.toLowerCase() ? categoryId : 6,
              colors: data.colors || [],
              sizes: data.sizes || [],
          ...
          .filter((product) => {
            if (product.stock < 10) {
              console.warn('Filtered out product with low stock (Games & Fun):', {
                id: product.id,
                name: product.name,
                stock: product.stock,
              });
            ...
            if (!isValidImage && product.imageUrl !== '/images/placeholder.jpg') {
              console.warn('Filtered out product with invalid imageUrl (Games & Fun):', {
                id: product.id,
                name: product.name,
        ...
        if (productsData.length === 0) {
          console.warn('No products passed the filters (Games & Fun). Relaxing stock filter to debug...');
          const relaxedProductsData = allProducts
            ...
            .filter((product) => {
              const isValidImage = product.imageUrl && typeof product.imageUrl === 'string' && product.imageUrl.startsWith('https://');
              if (!isValidImage && product.imageUrl !== '/images/placeholder.jpg') {
                console.warn('Filtered out product with invalid imageUrl (relaxed filter, Games & Fun):', {
                  id: product.id,
                  name: product.name,
                  imageUrl: product.imageUrl,
                });
          ...
          console.log('Products with relaxed stock filter (Games & Fun):', relaxedProductsData);
