...
export default function HomeKitchen() {
  ...
  const categoryId = 8;
  const firestoreCategory = 'home & kitchen';
  ...
  useEffect(() => {
    const fetchHomeKitchen = async () => {
      try {
        ...
        console.log('All fetched products (Home & Kitchen):', allProducts);
        ...
        const productsData = allProducts
          .map((doc) => {
            ...
            return {
              ...
              price: data.price || 0,
              stock: data.stock || 0,
              category: data.category || 'home & kitchen',
              categoryId: data.category?.trim().toLowerCase() === firestoreCategory.toLowerCase() ? categoryId : 8,
              colors: data.colors || [],
              sizes: data.sizes || [],
          ...
          .filter((product) => {
            if (product.stock < 10) {
              console.warn('Filtered out product with low stock (Home & Kitchen):', {
                id: product.id,
                name: product.name,
                stock: product.stock,
              });
            ...
            if (!isValidImage && product.imageUrl !== '/images/placeholder.jpg') {
              console.warn('Filtered out product with invalid imageUrl (Home & Kitchen):', {
                id: product.id,
                name: product.name,
        ...
        if (productsData.length === 0) {
          console.warn('No products passed the filters (Home & Kitchen). Relaxing stock filter to debug...');
          const relaxedProductsData = allProducts
            ...
            .filter((product) => {
              const isValidImage = product.imageUrl && typeof product.imageUrl === 'string' && product.imageUrl.startsWith('https://');
              if (!isValidImage && product.imageUrl !== '/images/placeholder.jpg') {
                console.warn('Filtered out product with invalid imageUrl (relaxed filter, Home & Kitchen):', {
                  id: product.id,
                  name: product.name,
                  imageUrl: product.imageUrl,
                });
          ...
          console.log('Products with relaxed stock filter (Home & Kitchen):', relaxedProductsData);
