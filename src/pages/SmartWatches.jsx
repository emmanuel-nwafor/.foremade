...
export default function SmartWatches() {
  ...
  const category = 'Smart Watches';
  const categoryId = 9;
  const firestoreCategory = 'smart watches';
  ...
  useEffect(() => {
    const fetchSmartWatches = async () => {
      try {
        ...
        console.log('All fetched products (Smart Watches):', allProducts);
        ...
        const productsData = allProducts
          .map((doc) => {
            ...
            return {
              ...
              price: data.price || 0,
              stock: data.stock || 0,
              category: data.category || 'smart watches',
              categoryId: data.category?.trim().toLowerCase() === firestoreCategory.toLowerCase() ? categoryId : 9,
              colors: data.colors || [],
              sizes: data.sizes || [],
          ...
          .filter((product) => {
            if (product.stock < 10) {
              console.warn('Filtered out product with low stock (Smart Watches):', {
                id: product.id,
                name: product.name,
                stock: product.stock,
              });
            ...
            if (!isValidImage && product.imageUrl !== '/images/placeholder.jpg') {
              console.warn('Filtered out product with invalid imageUrl (Smart Watches):', {
                id: product.id,
                name: product.name,
        ...
        if (productsData.length === 0) {
          console.warn('No products passed the filters (Smart Watches). Relaxing stock filter to debug...');
          const relaxedProductsData = allProducts
            ...
            .filter((product) => {
              const isValidImage = product.imageUrl && typeof product.imageUrl === 'string' && product.imageUrl.startsWith('https://');
              if (!isValidImage && product.imageUrl !== '/images/placeholder.jpg') {
                console.warn('Filtered out product with invalid imageUrl (relaxed filter, Smart Watches):', {
                  id: product.id,
                  name: product.name,
                  imageUrl: product.imageUrl,
                });
          ...
          console.log('Products with relaxed stock filter (Smart Watches):', relaxedProductsData);
