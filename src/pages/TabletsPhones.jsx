...
export default function TabletsPhones() {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  const category = 'Tablet & Phones';
  const categoryId = 1;
  const firestoreCategory = 'tablet & phones';

  useEffect(() => {
    const fetchTabletsPhones = async () => {
      try {
        setLoading(true);
        setError(null);
        ...
        console.log('All fetched products (Tablet & Phones):', allProducts);
        ...
        const productsData = allProducts
          .map((doc) => {
            ...
            return {
              ...
              price: data.price || 0,
              stock: data.stock || 0,
              category: data.category || 'tablet & phones',
              categoryId: data.category?.trim().toLowerCase() === firestoreCategory.toLowerCase() ? categoryId : 1,
              colors: data.colors || [],
              sizes: data.sizes || [],
          ...
          .filter((product) => {
            if (product.stock < 10) {
              console.warn('Filtered out product with low stock (Tablet & Phones):', {
                id: product.id,
                name: product.name,
                stock: product.stock,
              });
            ...
            if (!isValidImage && product.imageUrl !== '/images/placeholder.jpg') {
              console.warn('Filtered out product with invalid imageUrl (Tablet & Phones):', {
                id: product.id,
                name: product.name,
        ...
        if (productsData.length === 0) {
          console.warn('No products passed the filters (Tablet & Phones). Relaxing stock filter to debug...');
          const relaxedProductsData = allProducts
            ...
            .filter((product) => {
              const isValidImage = product.imageUrl && typeof product.imageUrl === 'string' && product.imageUrl.startsWith('https://');
              if (!isValidImage && product.imageUrl !== '/images/placeholder.jpg') {
                console.warn('Filtered out product with invalid imageUrl (relaxed filter, Tablet & Phones):', {
                  id: product.id,
                  name: product.name,
                  imageUrl: product.imageUrl,
                });
          ...
          console.log('Products with relaxed stock filter (Tablet & Phones):', relaxedProductsData);
