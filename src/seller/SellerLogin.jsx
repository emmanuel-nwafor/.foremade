...
export default function SellerLogin() {
  ...
  const handleSocialLogin = async (user) => {
    try {
      const vendorDoc = doc(db, 'vendors', user.uid);
      const vendorSnapshot = await getDoc(vendorDoc);
      if (vendorSnapshot.exists()) {
        const vendorData = vendorSnapshot.data();
        localStorage.setItem('vendorData', JSON.stringify(vendorData));
        setSuccessMessage(`Welcome back, ${vendorData.name.split(' ')[0]}!`);
        setTimeout(() => {
          setLoadingGoogle(false);
          setLoadingFacebook(false);
          navigate('/overview'); // Adjust redirect path as needed
        }, 2000);
      } else {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const userCredential = await signInWithEmailAndPassword(vendorAuth, email, password);
